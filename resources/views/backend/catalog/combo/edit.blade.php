                    ...
                    <div class="col-md-12">
                        <div class="card card-info">
                            <div class="card-header">
                                <h3 class="card-title">Chỉnh sửa Combo</h3>                               
                            </div>
                            <div class="card-body">                           
                                <div class="row">
                                    ...
                                    <div class="col-md-8">
                                        <div class="form-group">
                                            ...
                                            <input type="text" name="name" class="form-control" placeholder="Nhập tên Combo" value="{{old('name', $edit->name)}}">
                                        </div> 
                                        <div class="row">
                                            <div class="col-md-6">
                                                <div class="form-group">
                                                    <label>Giá bán</label>
                                                    <input type="number" name="price" class="form-control" placeholder="Nhập giá bán Combo" value="{{old('price', $edit->price)}}">
                                                </div> 
                                            </div>
                                            <div class="col-md-6">
                                                <div class="form-group">
                                                    <label>Giá vốn</label>
                                                    <input type="number" name="cost_price" id="cost_price_data" class="form-control" readonly value="{{old('cost_price', $edit->cost_price)}}">
                                                </div> 
                                            </div>
                                            <div class="col-md-12">
                                                <div class="form-group">
                                                    <label>Mô tả thêm</label>
                                                    <textarea class="form-control mb-3" name="description" placeholder="Nhập mô tả thêm" style="height: 70px">{{ old('description', $edit->description) }}</textarea>
                                                </div> 
                                            </div>
                                            <div class="col-md-12">
                                                ...
                                                <input type="hidden" id="combo-items-data" value="{{ json_encode($combo_items) }}">
                                                <input type="hidden" id="combo-id-edit" value="{{ $edit->id }}">
                                            </div>
                                        </div>                                                                  
                                    </div>
                                </div>
...
@section('scripts')
    ...
    <script>
        $(document).ready(function () {
            ...
            var ids = {};
            var total_price = 0;
            var existingItems = JSON.parse($("#combo-items-data").val());
            existingItems.forEach(item => {
                ids[item.item_id] = {
                    image: item.item.avatar,
                    name: item.item.name,
                    salePrice: item.item.active_price.sale_price.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ".") + " đ",
            ...
            Object.keys(ids).forEach(itemId => {
                ...
                newItems.push([
                    ...
                    item.category,
                    `<button class="btn btn-sm btn-remove-item" data-id="${itemId}">
                        <i class="fa-solid fa-xmark"></i>
                    </button>`,
                    itemId
                ]);
            });
            table.rows.add(newItems).draw();
            table.rows().every(function () {
                let row = this.node();
                let itemId = this.data()[6];
