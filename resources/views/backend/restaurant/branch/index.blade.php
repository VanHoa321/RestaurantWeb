                ...
                <div class="row mb-2">
                    <div class="col-sm-6">
                        <ol class="breadcrumb float-sm-left">
                            ...
                            <li class="breadcrumb-item active text-info"><a href="{{ route('branch.index') }}" class="text-info">Cơ sở</a></li>
                        </ol>
                    </div>
                </div>
            </div>
                    ...
                    <div class="col-12">
                        <div class="card">
                            <div class="card-header">
                                <div>
                                    <a type="button" class="btn btn-success" href="{{route('branch.create')}}">
                                        <i class="fa-solid fa-plus" title="Thêm mới cơ sở"></i>
                                    </a>
                                </div>
                            </div>
                            <div class="card-body">
                                    ...
                                    <tbody>
                                        ...
                                        @foreach ($items as $item)
                                            <tr id="branch-{{ $item->id }}">
                                                ...
                                                <td>
                                                    <a href="{{route('branch.edit', $item->id)}}" class="btn btn-info btn-sm" title="Sửa thông tin cơ sở">
                                                        <i class="fa-solid fa-pen-to-square"></i>
                                                    </a>
                                                    <a href="#" class="btn btn-danger btn-sm btn-delete" data-id="{{ $item->id }}" title="Xóa cở sở nhà hàng">
    ...
    <script>
        $(document).ready(function() {
            ...
            $('body').on('change', '.IsActive', function(e) {
                ...
                const id = check.val();
                $.ajax({
                    url: "/restaurant/branch/change/" + id,
                    type: "POST",
                    data: {
                        _token: '{{ csrf_token() }}'
                    },
            ...
            $('body').on('click', '.btn-delete', function(e) {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: "/restaurant/branch/destroy/" + id,
                            type: "DELETE",
                            data: {
                                _token: '{{ csrf_token() }}'
                            },
                            ...
                            error: function(xhr) {
                                toastr.error('Có lỗi khi xóa cơ sở');
                            }
                        });
                    }
                });
