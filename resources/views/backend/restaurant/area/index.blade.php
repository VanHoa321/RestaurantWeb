                ...
                <div class="row mb-2">
                    <div class="col-sm-6">
                        <ol class="breadcrumb float-sm-left">
                            ...
                            <li class="breadcrumb-item active text-info"><a href="{{ route('branch.index') }}" class="text-info">Khu vực</a></li>
                        </ol>
                    </div>
                </div>
            </div>
                    ...
                    <div class="col-12">
                        <div class="card">
                            <div class="card-header">
                                <div>
                                    <a type="button" class="btn btn-success" href="{{route('area.create')}}">
                                        <i class="fa-solid fa-plus" title="Thêm mới khu vực"></i>
                                    </a>
                                </div>
                            </div>
                            <div class="card-body">
                                    ...
                                    <tbody>
                                        ...
                                        @foreach ($items as $item)
                                            <tr id="area-{{ $item->id }}">
                                                ...
                                                <td>
                                                    <a href="{{route('area.edit', $item->id)}}" class="btn btn-info btn-sm" title="Sửa thông tin khu vực">
                                                        <i class="fa-solid fa-pen-to-square"></i>
                                                    </a>
                                                    <a href="#" class="btn btn-danger btn-sm btn-delete" data-id="{{ $item->id }}" title="Xóa khu vực">
    ...
    <script>
        $(document).ready(function() {
            ...
            $('body').on('click', '.btn-delete', function(e) {
                ...
                Swal.fire({
                    title: "Xác nhận xóa khu vực này?",
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: "/restaurant/area/destroy/" + id,
                            type: "DELETE",
                            data: {
                                _token: '{{ csrf_token() }}'
                            },
                            ...
                            error: function(xhr) {
                                toastr.error('Có lỗi khi xóa khu vực');
                            }
                        });
                    }
                });
