                ...
                <div class="row">
                    <div class="col-md-12">
                        <div class="card card-info">
                            ...
                            <form method="post" action="{{route("area.update", $edit->id)}}" id="quickForm">
                                @csrf
                                <div class="card-body">                           
                                    <div class="form-group">
                                        <label>Tên khu vực</label>
                                        <input type="text" name="name" value="{{ old('name', $edit->name) }}" class="form-control" placeholder="Nhập tên khu vực">
                                    </div>
                                    <div class="form-group">
                                        <label>Thuộc cơ sở</label>
                                        <select name="branch_id" class="form-control select2bs4" style="width: 100%">
                                            @foreach($branchs as $branch)
                                                <option value="{{$branch->id}}" {{ old('branch_id', $branch->id) == $edit->branch_id ? 'selected' : '' }}>{{$branch->name}}, {{ $branch->address }}</option>
                                            @endforeach
                                        </select>
                                    </div>                                                            
                                    <div class="form-group">
